...
# apply panalty if value is out of range (min, max)
# https://www.desmos.com/calculator: y=\left(\max\left(0,\ x-1\right)+\max\left(0,-x-1\right)\right)^{2}
def relu_panalty(x, value_range=(0,1)):
    min = value_range[0]
    max = value_range[1]
    return torch.pow((torch.relu(x-max) + torch.relu(-x+min)), 2)
...
# std = torch.mean(torch.tensor(stds))
    ...
    def run(self) -> None:
        ...
        # threshod for dnbr
        self.tau = nn.Parameter(torch.tensor(0.1).to(self.DEVICE), requires_grad=True)#
        self.alpha = nn.Parameter(torch.tensor(0.5).to(self.DEVICE), requires_grad=True)#
        params = list(self.model.parameters()) + [self.tau, self.alpha]
        ...
        for epoch in range(0, self.cfg.MODEL.MAX_EPOCH):
            ...
            if valid_logs['total_loss'] < min_loss:
                min_loss = valid_logs['total_loss']
    ...
    def step(self, phase) -> dict:
        ...
        with tqdm(iter(self.dataloaders[phase]), desc=phase, file=sys.stdout, disable=not self.cfg.MODEL.VERBOSE) as iterator:
            for i, (x, y) in enumerate(iterator):
                ...
                # add dnbr loss
                def nbr(x): return (x[:,:1,] - x[:,-1:,]) / (x[:,:1,] + x[:,-1:,] + 1e-4)
                if self.cfg.DATA.STACKING:
                    x1 = input[0][:, :(input[0].shape[1]//2),]
                    x2 = input[0][:, (input[0].shape[1]//2):,]
                    # dnbr = torch.sigmoid((nbr(x1) - nbr(x2) - self.tau))
                    dnbr = torch.relu((nbr(x1) - nbr(x2) - self.tau))
                    dnbr = dnbr / (dnbr + 1e-8)
                dnbr_loss = self.criterion(out, dnbr)

                loss_ = self.alpha * loss_ \
                        + (1 - self.alpha) * dnbr_loss \
                        + 10 * torch.pow(self.tau - 0.1, 2) \
                        + 100 * relu_panalty(self.alpha, (0.4,0.6))
...
def run_app(cfg : DictConfig) -> None:
    ...
    wandb.init(config=cfg_flat, project='weakly-supervised-learning', entity=cfg.PROJECT.ENTITY, name=cfg.EXP.NAME)
