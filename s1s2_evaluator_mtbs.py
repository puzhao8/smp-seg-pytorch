...
def loc_channel(x):
    return list(x.shape).index(np.array(x.shape).min())
...
def inference(model, test_dir, test_id, cfg):
    ...
    for sat in cfg.DATA.SATELLITES:
        ...
        if 2 == loc_channel(post_image): post_image = post_image.transpose(2,0,1) # HWC -> CHW
        ...
        if 'pre' in cfg.DATA.PREPOST:
            if 'prg' == cfg.EVAL.MODE: # NOT same filenames in pre and post folder
                query = test_id.split("_")[-1]
                pre_url = glob.glob(str(test_dir / sat / "pre" / f"*_{query}.tif"))[0]
                print("pre_image: ", os.path.split(pre_url)[-1])

            else: # same filename in pre and post folder
                pre_url = test_dir / sat / "pre" / f"{test_id}.tif"

            pre_image = tiff.imread(pre_url) # CHW
            if 2 == loc_channel(pre_image): pre_image = pre_image.transpose(2,0,1) # HWC -> CHW
...
def gen_burn_severity_map(grouthTruth, preMap, save_url=False):
    # errMap = np.zeros(preMap.shape)
    # # errMap[np.where((OptREF==0) & (SARREF==0))] = 0
    # errMap[np.where((grouthTruth==1) & (preMap==1))] = 1.0 # TP, dark red
    # errMap[np.where((grouthTruth==1) & (preMap==0))] = 2.0 # FN, light red
    ...
    num_color = int(1 + max(np.unique(preMap)))
    # color_tuple = ([1,1,1], [0.6,0,0], [0,0.8,0], [1, 0.6, 0.6])
    color_tuple = ([1,1,1], [0.5,1,0.83], [1, 1, 0], [1,0,0])
    my_cmap = ListedColormap(color_tuple[:num_color])
    ...
    if save_url:
        plt.imsave(save_url, preMap, cmap=my_cmap)

        saveName = os.path.split(save_url)[-1].split('.')[0]
        errMap_rgb = imread(save_url)
        wandb.log({f"test_preMap/{saveName}": wandb.Image(errMap_rgb)})
    return preMap
