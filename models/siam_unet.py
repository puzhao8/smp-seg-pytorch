...
class SiamUnet_conc(nn.Module):
    ...
    def __init__(self,
        ...
        decoder_topo = topo[::-1]
        decoder_topo = [2*de_topo for de_topo in decoder_topo]
        self.decode = Decoder(align_corners, use_deconv=use_deconv, decoder_topo=decoder_topo)
        self.cls = self.conv = nn.Conv2d(
            in_channels=topo[0]*2, # *2 only for SiamUnet_conc
...
class SiamUnet_diff(nn.Module):
    ...
    def forward(self, x, train=True):
        ...
        xc = torch.sub(xc1, xc2) # feat diff

        short_cut = []
        for cut1, cut2 in zip(short_cuts1, short_cuts2):
            stacked_feat = torch.sub(cut1, cut2) # feat diff
            short_cut.append(stacked_feat)
...
class Decoder(nn.Module):
    def __init__(self, align_corners, use_deconv=False, decoder_topo=[512,256,128,64]):
        super().__init__()
